...
class BenchmarkResult:
    def __init__(self, name, elapsed, mean, std):
        self.name = name
...
def measure(repeat=1):
    def _measure(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            _results.append(BenchmarkResult(f'{func.__name__}({all_args})', (a - b) / repeat, 0, 0)) # TODO: calculate mean and std
            return res

        return wrapper
    return _measure

def _float2str(f): return f'{f:.4f}'
...
def print_results(title=None):
    ...
    columns = ['Name', 'Elapsed Time', 'Mean', 'Std']
    table = [columns, *[[res.name, _float2str(res.elapsed), _float2str(res.mean), _float2str(res.std)] for res in _results]]
    ...
    max_vars = list(map(_float2str, [max_elapsed, max_mean, max_std]))
    min_vars = list(map(_float2str, [min_elapsed, min_mean, min_std]))
